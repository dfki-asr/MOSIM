...
REM start cmd.exe /c "docker run -p %CSharpAdapter%:%DOCKER_STDP% -t csharpadapter:latest -a %MOSIM_IPA%:%CSharpAdapter% -aint %DOCKER_STD_IPP% -r %MOSIM_IPA%:%LAUNCHER% -m ../../MMUs" 
REM timeout /t 5

start cmd.exe /c "docker run -p %CoSimulation%:%DOCKER_STDP% -p %CoSimulationServ%:%CoSimulationServInt% -t cosimulation:latest -a %MOSIM_IPA%:%CoSimulation% -aint %DOCKER_STD_IPP% -aserv %MOSIM_IPA%:%CoSimulationServ% -aservint %MOSIM_IPA%:%CoSimulationServInt%  -r %MOSIM_IPA%:%LAUNCHER%" 
REM timeout /t 5
...
REM start cmd.exe /c "docker run -p %CoordinateSystemMapper%:%DOCKER_STDP% -t coordinatesystemmapper:latest -a %MOSIM_IPA%:%CoordinateSystemMapper% -aint %DOCKER_STD_IPP% -r %MOSIM_IPA%:%LAUNCHER%"
REM timeout /t 5

REM start cmd.exe /c "docker run -p %PostureBlending%:%DOCKER_STDP%" -t postureblending:latest -a %MOSIM_IPA%:%PostureBlending% -aint %DOCKER_STD_IPP% -r %MOSIM_IPA%:%LAUNCHER%"
REM timeout /t 5 
