...
REM start cmd.exe /c "docker run -p %LAUNCHER%:%LAUNCHER% -it -v  launcher:latest -a %MOSIM_IPA%:%LAUNCHER%"
REM timeout /t 10
...
REM start cmd.exe /c "docker run -p %CSharpAdapter%:%DOCKER_STDP% -t csharpadapter:latest %MOSIM_IPA% %DOCKER_STDP% %CSharpAdapter% %LAUNCHER% -m ../../MMUs" 
REM timeout /t 5
...
REM start cmd.exe /c "docker run -p %UnityAdapter%:%DOCKER_STDP% -t unityadapter %MOSIM_IPA% %DOCKER_STDP% %UnityAdapter% %LAUNCHER%"
REM timeout /t 5
